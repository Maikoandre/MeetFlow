...
def cancelar_inscricao(request, evento_id):
    evento = get_object_or_404(Evento, pk=evento_id)
    inscricao = Inscricao.objects.filter(evento=evento, participante=request.user).first()
    ...
    return render(request, 'forms/cancelar_inscricao.html', {'evento': evento})

@login_required
def confirmar_inscricao(request, token):
    inscricao = get_object_or_404(Inscricao, status='pendente', participante=request.user)
    inscricao.status = 'confirmado'
    inscricao.save()
    messages.success(request, "Inscrição confirmada com sucesso!")
    return redirect('detalhes_evento', pk=inscricao.evento.id)
